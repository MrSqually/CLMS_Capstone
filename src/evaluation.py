        ...
        for init_ctx in ("none", "qa", "halu"):
            for ctx in ("base", "contradict", "instruct", "chain-thought"):
                ...
                for metric, values in question_bertscores.items():
                    ...
                    halu_var = np.average(val_array, axis=0) - expected_val
                    ...
                    plt.savefig(f"results/figures/{model_name}/{init_ctx}-{ctx}-{metric}-hv.png")
                    plt.clf()
                    plt.plot(val_array.T)
                    plt.savefig(f"results/figures/{model_name}/{init_ctx}-{ctx}-{metric}-bertscore.png")
                    plt.clf()
                    var_df.var(axis=0).plot(title=f"{init_ctx}-{ctx}-{metric}")
                    plt.savefig(f"results/figures/{model_name}/{init_ctx}-{ctx}-{metric}-regular-variance.png")
                    plt.clf()
                    plt.close()
...
if __name__ == "__main__":
    halu = HaluMetrics()
    halu.get_haluvar("results/prompting/mixtral")
